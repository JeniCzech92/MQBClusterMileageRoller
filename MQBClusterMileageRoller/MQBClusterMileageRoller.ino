...
unsigned char P_L_CC_CRC_LUT_APV[256] = { 0, 47, 94, 113, 188, 147, 226, 205, 87, 120, 9, 38, 235, 196, 181, 154, 174, 129, 240, 223, 18, 61, 76, 99, 249, 214, 167, 136, 69, 106,
              ...
              P_L_CC_KENNUNG_APV_AIRBAG01[16] = { 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40 },
              P_L_CC_KENNUNG_APV_KlemmenStatus01[16] = { 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3, 0xc3 },
              P_L_CC_KENNUNG_APV_ESP02[16] = { 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA },
              P_L_CC_KENNUNG_APV_ESP10[16] = { 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac, 0xac },
              P_L_CC_KENNUNG_APV_ESP20[16] = { 0xAC, 0xB3, 0xAB, 0xEB, 0x7A, 0xE1, 0x3B, 0xF7, 0x73, 0xBA, 0x7C, 0x9E, 0x06, 0x5F, 0x02, 0xD9 },
              P_L_CC_KENNUNG_APV_ESP21[16] = { 0xb4, 0xef, 0xf8, 0x49, 0x1e, 0xe5, 0xc2, 0xc0, 0x97, 0x19, 0x3c, 0xc9, 0xf1, 0x98, 0xd6, 0x61 },
              P_L_CC_KENNUNG_APV_ESP24[16] = { 0x67, 0x8A, 0xAE, 0x22, 0x4D, 0xD0, 0x51, 0x80, 0x5C, 0xB9, 0xCE, 0x1E, 0xDF, 0x02, 0x2D, 0xD4 },
              P_L_CC_KENNUNG_APV_TSK07[16] = { 0x78, 0x68, 0x3A, 0x31, 0x16, 0x08, 0x4F, 0xDE, 0xF7, 0x35, 0x19, 0xE6, 0x28, 0x2F, 0x59, 0x82 },
              P_L_CC_KENNUNG_APV_LH_EPS01[16] = { 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29, 0x29 },
              P_L_CC_KENNUNG_APV_WBA03[16] = { 0x47, 0x94, 0x92, 0x6A, 0x67, 0xB5, 0x0D, 0x38, 0xE3, 0x8A, 0x5D, 0xB4, 0x54, 0xAB, 0xAE, 0x27 },
              P_L_CC_KENNUNG_APV_MCODE01[16] = { 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47, 0x47 },
              P_L_CC_KENNUNG_APV_SWA01[16] = { 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C },
              P_L_CC_KENNUNG_APV_LICHT_ANF[16] = { 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07 },

              kStatusBuf[4] = { 0x00, 0x00, 0x03, 0x00 },
...
uint32_t targetMileage = 0,
         ...
         lastPoll = 0,
         lastStep = 0,
         progressFromMileage = 0,
         progressbar_end = 4000,        //most practical. 1000 = 25% progress
         SPEED = 315;
...
void sendSteeringWheelControls(int button) {
  switch (button) {
    case 1:
      mfswBuf[0] = 0x01;
      mfswBuf[2] = 0x01;
      break;  // Menu (does nothing)
    case 2:
      mfswBuf[0] = 0x02;
      mfswBuf[2] = 0x01;
      break;  // Right (does nothing)
    case 3:
      mfswBuf[0] = 0x03;
      mfswBuf[2] = 0x01;
      break;  // Left (does nothing)
    case 4:
      mfswBuf[0] = 0x06;
      mfswBuf[2] = 0x01;
      break;  // Up
    ...
    case 7:
      mfswBuf[0] = 0x21;
      mfswBuf[2] = 0x01;
      break;  // Asterisk (does nothing)
    case 8:
      mfswBuf[0] = 0x23;
      mfswBuf[2] = 0x01;
      break;  // View (does nothing)
  }
...
void sendDoorStatus(boolean drvDoor, boolean passDoor = false, boolean rDrvDoor = false, boolean rPassDoor = false, boolean trunk = false) {
  doorStatusBuff[3] = 0;
  // bit 0: left front, bit 1: right front, bit 2: left rear, bit 3: right rear, bit 4: trunk
  if (drvDoor) doorStatusBuff[3]++;
  if (passDoor) doorStatusBuff[3] += 2;
  if (rDrvDoor) doorStatusBuff[3] += 4;
  if (rPassDoor) doorStatusBuff[3] += 8;
  if (trunk) doorStatusBuff[3] += 16;
...
void handleCommand(String command) {
  if (command == F("run")) {
    ...
    taskRunning = true;
    EEPROM.update(EEPROM_ADDR_RUNNING, 1);
  } else if (command == "stop") {
    Serial.println(F("Stopping..."));
    taskRunning = false;
    EEPROM.update(EEPROM_ADDR_RUNNING, 0);
  } else if (command.startsWith(F("mil "))) {
    if (command.length() > 4) {
      uint32_t val = command.substring(4).toInt();
      if (val != 0 && val < 1000000) {
        Serial.print(F("New target mileage: "));
        Serial.println(val);
        EEPROM.put(EEPROM_ADDR_MILEAGE, val);
        targetMileage = val;
      } else {
        Serial.println(F("Invalid input. Values [1-999999] are accepted."));
      }
    }
  } else if (command == F("stat")) {
    Serial.println(taskRunning ? F("Task is currently running.") : F("Task is currently suspended."));
    Serial.print(F("Current mileage: "));
    Serial.println(currentMileage);
    Serial.print(F("Target mileage: "));
    Serial.println(targetMileage);
    Serial.println(EEPROM.read(EEPROM_ADDR_RELATIVE) ? F("Progress shown relatively") : F("Progress shown absolutely."));
  } else if (command == F("rel")) {
    Serial.println(F("Progress shown relative (from current mileage)"));
    EEPROM.update(EEPROM_ADDR_RELATIVE, 1);
    progressFromMileage = currentMileage;
  } else if (command == F("abs")) {
    Serial.println(F("Progress shown absolute (from 0km)"));
    EEPROM.update(EEPROM_ADDR_RELATIVE, 0);
    progressFromMileage = 0;
  } else {
    if (command != F("help")) Serial.println(F("Unknown request."));
    Serial.println(F("Available commands:"));
    Serial.println(F("run - Starts rolling the odometer"));
    Serial.println(F("stop - Stops rolling the odometer"));
    Serial.println(F("mil [1-999999] - Sets target mileage. ATTENTION! Rolling over 999999 will NOT zero the cluster!"));
    Serial.println(F("stat - Shows current status"));
    Serial.println(F("abs - The RPM 'progress bar' will show progress from 0km"));
    Serial.println(F("rel - The RPM 'progress bar' will show progress from current mileage"));
...
void setup() {
  ...
  EEPROM.get(EEPROM_ADDR_MILEAGE, targetMileage);
  if (EEPROM.read(EEPROM_ADDR_RUNNING)) {
    ;
    taskRunning = true;
    Serial.print(F("Resuming task. Target mileage: "));
    Serial.println(targetMileage);
  } else Serial.println(F("Ready for commands."));
...
void loop() {
  ...
  if (now - lastStep >= STEP_INTERVAL) {
    ...
    if (taskRunning) {
      ...
      sendMotor(map(currentMileage, progressFromMileage, targetMileage, 0, progressbar_end), 90);  //RPM and coolant data
      sendESP24();
      sendTSK07();
      sendLhEPS01();
      ...
      uint8_t laneAssist[8] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
      CAN.sendMsgBuf(0x3F3, 0, 8, laneAssist);
  ...
  if (CAN.checkReceive() == CAN_MSGAVAIL) {
    ...
    if (id == 0x77E && buf[0] == 0x06 && buf[1] == 0x62) {
      currentMileage = ((uint32_t)buf[4] << 16) | ((uint32_t)buf[5] << 8) | (uint32_t)buf[6];
      if (progressFromMileage == 0 && EEPROM.read(EEPROM_ADDR_RELATIVE)) progressFromMileage = currentMileage;
      if (currentMileage >= targetMileage) {
        Serial.print(F("Target reached. "));
        handleCommand(F("stop"));
      }
    }
  }
